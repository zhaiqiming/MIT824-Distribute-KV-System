...
func (rf *Raft) persist() {
	...
	_ = e.Encode(rf.log) //only three presistent state on all server
	_ = e.Encode(rf.timestamp)
	data := w.Bytes()
	rf.persister.SaveRaftState(data)
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) {
	...
	if args.Term >= rf.currentTerm {
		...
		if args.TimeStamp <= rf.timestamp {
			...
			//fmt.Println(rf.me, "this appeAppendEntriesnd entries rpc is timeout")
...
func (rf *Raft) SaveSnapshot(index int, kvdatabase map[string]string, detectDup  map[int64]int) {
	...
	w := new(bytes.Buffer)
	e := labgob.NewEncoder(w)
	_ = e.Encode(kvdatabase)
	_ = e.Encode(detectDup)
	data := w.Bytes()
...
func Make(peers []*labrpc.ClientEnd, me int,
	persister *Persister, applyCh chan ApplyMsg) *Raft { //applyMsg is used to insure that same CommandIndex has same entry
	rf := &Raft{}
	rf.peers = peers
