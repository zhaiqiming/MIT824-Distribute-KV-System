		...
		if msg.CommandValid {
			...
			if oop, ok := msg.Command.(Op); ok {
				...
				if kv.DupCheck(oop.ClientId, oop.Seq) {
					switch oop.Opname {
					case "Put":
						kv.Putkvdatabase(oop)
					case "Append":
						kv.Appendkvdatabase(oop)
					}
...
func (kv *KVServer) Putkvdatabase(op Op){
	kv.kvdatabase[op.Key] = op.Value
	fmt.Println(kv.me, "after put", kv.kvdatabase[op.Key][max(len(kv.kvdatabase[op.Key]) - 30, 0): ])
}
func (kv *KVServer) Appendkvdatabase(op Op){
	if _, ok := kv.kvdatabase[op.Key]; ok {
		kv.kvdatabase[op.Key] += op.Value
		fmt.Println(kv.me, "after append", kv.kvdatabase[op.Key][max(len(kv.kvdatabase[op.Key]) - 30, 0): ])
	} else {
		kv.kvdatabase[op.Key] = op.Value
		fmt.Println(kv.me, "after append", kv.kvdatabase[op.Key][max(len(kv.kvdatabase[op.Key]) - 30, 0): ])
	}
